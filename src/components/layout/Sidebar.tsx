...
import { getMockAccessContext } from "../../services/accessContext.service";
...
export function Sidebar({ role, mobileOpen, collapsed, close, toggleCollapsed }: { role: UserRole; mobileOpen: boolean; collapsed: boolean; close: () => void; toggleCollapsed: () => void }) {
  const { user, enabledClinicModules } = getMockAccessContext(role);
  const navigation = getNavigationForUser(user, enabledClinicModules, user.modules, user.permissions);
