import { Bell, LogOut, PanelLeftClose, PanelLeftOpen, Search, Menu } from "lucide-react";
...
export function Topbar({ role, openMenu, collapsed, toggleCollapsed }: { role: UserRole; openMenu: () => void; collapsed: boolean; toggleCollapsed: () => void }) {
  ...
  return <header className="sticky top-0 z-20 flex min-h-[72px] items-center border-b bg-white/95 px-4 backdrop-blur md:px-6">
    ...
    <div className="ml-auto flex items-center gap-2"><RoleSwitcher role={role} /><Button size="icon" variant="secondary" aria-label="Notifications" icon={<Bell className="size-4" />} /><Button size="icon" variant="secondary" aria-label="Log out" onClick={logout} loading={loggingOut} icon={<LogOut className="size-4" />} /><div className="grid size-10 place-items-center rounded-lg bg-brand-100 text-sm font-bold text-brand-700">{role === "doctor" ? "DR" : role === "super_admin" ? "SA" : "VS"}</div></div>
  </header>;
}
