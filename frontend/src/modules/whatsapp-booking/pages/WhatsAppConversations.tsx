...
export default function WhatsAppConversations() {
  const [query, setQuery] = useState(""); const [status, setStatus] = useState<WhatsAppConversationStatus | "all">("all"); const [source, setSource] = useState<WhatsAppConversation["source"] | "all">("all"); const [selected, setSelected] = useState<WhatsAppConversation | null>(null); const [conversations, setConversations] = useState<WhatsAppConversation[]>([]); const [phone, setPhone] = useState(""); const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false);
  ...
  useEffect(load, []);
  const rows = useMemo(() => conversations.filter((conversation) => (status === "all" || conversation.status === status) && (source === "all" || conversation.source === source) && `${conversation.patientName ?? ""} ${conversation.phone} ${conversation.lastMessage}`.toLowerCase().includes(query.toLowerCase())), [query, status, source, conversations]);
  ...
  return <div className="space-y-5"><PageHeader title="WhatsApp Conversations" description="Track patient chats, booking steps, handoffs and failed-message placeholders." /><Card className="p-4"><div className="grid gap-3 md:grid-cols-[1fr_auto]"><Input placeholder="New WhatsApp phone number" value={phone} onChange={(event) => setPhone(event.target.value)} /><Button loading={saving} onClick={() => void createConversation()}>Create Conversation</Button></div></Card><Card className="p-4"><div className="grid gap-3 lg:grid-cols-3"><Input placeholder="Search patient name/phone" value={query} onChange={(e) => setQuery(e.target.value)} /><Select value={status} onChange={(e) => setStatus(e.target.value as WhatsAppConversationStatus | "all")}><option value="all">All statuses</option><option value="new">New</option><option value="in_progress">In progress</option><option value="appointment_confirmed">Appointment confirmed</option><option value="waiting_for_patient">Waiting for patient</option><option value="transferred_to_reception">Transferred</option><option value="closed">Closed</option><option value="failed">Failed</option></Select><Select value={source} onChange={(e) => setSource(e.target.value as WhatsAppConversation["source"] | "all")}><option value="all">All sources</option><option value="whatsapp">WhatsApp</option><option value="qr">QR</option><option value="website">Website</option></Select></div></Card>{loading ? <Card className="p-5 text-sm text-slate-500">Loading conversations...</Card> : rows.length === 0 ? <WhatsAppEmptyState title="No conversations found" description="Try a different search term or status filter." /> : <ConversationList conversations={rows} onView={setSelected} />}<Modal open={!!selected} onClose={() => setSelected(null)} title="Conversation Preview">{selected && <ConversationPreview conversation={selected} loading={saving} onTransfer={() => void transfer()} onCloseConversation={() => void closeConversation()} onSendPlaceholder={(body) => void sendPlaceholder(body)} />}</Modal></div>;
}
