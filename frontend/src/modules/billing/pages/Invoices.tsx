...
export default function Invoices() {
  ...
  return <div className="space-y-5"><PageHeader title="Invoices" description="Manage invoices, payment states, print/send/refund placeholders." /><Card className="p-4"><div className="grid gap-3 lg:grid-cols-5"><Input placeholder="Search invoice" value={q} onChange={(e) => setQ(e.target.value)} /><Select value={type} onChange={(e) => setType(e.target.value as BillType | "all")}><option value="all">All bill types</option><option value="consultation">Consultation</option><option value="procedure">Procedure</option><option value="pharmacy">Pharmacy</option><option value="package">Package</option><option value="other">Other</option></Select><Select value={status} onChange={(e) => setStatus(e.target.value as PaymentStatus | "all")}><option value="all">All statuses</option><option value="paid">Paid</option><option value="pending">Pending</option><option value="partial">Partial</option><option value="refunded">Refunded</option><option value="cancelled">Cancelled</option></Select><Select value={mode} onChange={(e) => setMode(e.target.value as PaymentMode | "all")}><option value="all">All modes</option><option value="cash">Cash</option><option value="upi">UPI</option><option value="card">Card</option><option value="online_link">Online Link</option></Select><Input type="date" defaultValue="2026-07-01" /></div></Card><Card className="p-5"><InvoiceTable invoices={rows} onView={setSelected} /></Card><Modal open={!!selected} onClose={() => setSelected(null)} title="Invoice Preview">{selected && <InvoicePreview invoice={selected} />}</Modal></div>;
}
